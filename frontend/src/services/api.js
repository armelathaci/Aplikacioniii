...
export const createTransaction = (transactionData) => 
  fetchApi('/transaction/create', { method: 'POST', body: JSON.stringify(transactionData) });

export const updateTransaction = (transactionId, transactionData) => 
  fetchApi(`/transaction/update/${transactionId}`, { method: 'PUT', body: JSON.stringify(transactionData) });

export const deleteTransaction = (transactionId) => 
  fetchApi(`/transaction/delete/${transactionId}`, { method: 'DELETE' });

// --- Goals Functions ---
export const getGoals = () => fetchApi('/goal/list');

export const createGoal = (goalData) => 
  fetchApi('/goal/create', { method: 'POST', body: JSON.stringify(goalData) });

export const updateGoal = (goalId, goalData) => 
  fetchApi(`/goal/update/${goalId}`, { method: 'PUT', body: JSON.stringify(goalData) });

export const deleteGoal = (goalId) => 
  fetchApi(`/goal/delete/${goalId}`, { method: 'DELETE' });

// --- Settings / User Profile Functions ---
export const getProfile = () => fetchApi('/user/profile');

export const updateProfile = (profileData) => 
  fetchApi('/user/profile', { method: 'PUT', body: JSON.stringify(profileData) });

export const changePassword = (passwordData) => 
  fetchApi('/user/password', { method: 'POST', body: JSON.stringify(passwordData) });

export const deleteAccount = () => 
  fetchApi('/settings/delete-account', { method: 'DELETE' });

// --- Original Settings API Calls ---
export const getSettings = () => fetchApi('/settings/notifications');

export const updateSettings = (data) => 
  fetchApi('/settings/notifications', { method: 'PUT', body: JSON.stringify(data) });

// --- AI Chat Functions ---
export const startAIChat = (topic = 'General') => 
  fetchApi('/ai-chat/start', { method: 'POST', body: JSON.stringify({ topic }) });

export const sendMessageToAI = async (conversationId, message) => {
  const data = await fetchApi('/ai-chat/message', { 
    method: 'POST', 
    body: JSON.stringify({ conversationId, message }) 
  });

  // Filtrim i mesazhit default nga backend
  if (data.aiResponse && data.aiResponse.content &&
      data.aiResponse.content.includes("AI brain is not configured")) {
    data.aiResponse.content = "Për momentin nuk mora përgjigje nga AI.";
  }
