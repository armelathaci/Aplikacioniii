...
class AIChatRoutes {
    ...
    async sendMessage(req, res, { databaseManager }) {
        try {
            ...
            if (!history) {
                const previousMessages = await databaseManager.all('SELECT role, content FROM ai_messages WHERE conversation_id = ? ORDER BY created_at ASC', [conversationId]);
                history = { messages: previousMessages, context: conversation.context, model: conversation.model };
            ...
            this.sendSuccess(res, 200, { 
                ...
                conversationStats: {
                    ...
                    totalTokens: history.messages.reduce((sum, msg) => sum + (msg.tokens || 0), 0)
                }
            });
        } catch (error) {
            ErrorHandler.logError(error, req);
